...
class ClassificationMetric(BinaryLabelDatasetMetric):
    ...
    第一个数据集是原始的数据集，第二个数据集是transformer(或 similar 类似的) 的分类的输出
    """
    ...
    def __init__(self, dataset, classified_dataset,
        ...
        """
        参数:
            dataset (BinaryLabelDataset): 包含了基准真实(ground-truth)标签的数据集.
            classified_dataset (BinaryLabelDataset): 包含了预测的数据集.
            privileged_groups (list(dict)): 受保护组。格式是一个'dicts'的列表，其中键是'protected_attribute_names'（保护属性的姓名），
            值是'protected_attributes'中的值。每个'dict'元素描述一个组。更多细节描述参考案例.
       ...
       抛出异常:
            类型错误：'dataset'和`classified_dataset`必须是'aif360.datasets.BinaryLabelDataset'（二分类标签数据集）类型
        """
        ...
        # 验证两个数据集除预测和元数据(predictions and metadata，即labels和scores)外的所有内容是否相同.
    ...
    def equal_opportunity_difference(self):
        ...
        return 0.5 * (self.difference(self.false_negative_rate)
                    + self.difference(self.true_positive_rate))
